        ...
        <tbody>
            @foreach ($blacklists as $blacklist)
                <tr id="tr">
                    ...
                    <td>
                        <button type='button' class='btn btn-secondary' style='margin-left: 5px;' id="btn-delele-phone-{{$blacklist['id']}}" onclick="deletePhone('{{$blacklist['id']}}')">Deletar</button>
                    </td>
                </tr>
            @endforeach
        </tbody>
...
<script>
    ...
    function deletePhone(id) {
        $(`#btn-delele-phone-${id}`).text('Deletando...');
        $(`#btn-delele-phone-${id}`).prop('disabled', true);
        var baseUrlDelete = "{{route('blacklist.destroy', ['blacklist'=>'idUser'])}}";
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if (result['error']) {
                    swal(
                        'Falha',
                        'Erro ao excluir número blacklist',
                        'warning'
                    );
                }
                swal(
                    'Sucesso',
                    'Número blacklist excluído com sucesso',
                    'success'
                );
                filtrar(true);
            },
            error: function (result) {
                ...
                $(`#btn-delele-phone-${id}`).text('Deletar');
                $(`#btn-delele-phone-${id}`).prop('disabled', false);
            }
        });
    }
</script>
