    ...
    <div class="page-wrapper">
        <div id="conteudo_animated">
                        ...
                        <div class="d-flex align-items-center">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb m-0 p-0">
                                    ...
                                    <li class="breadcrumb-item text-muted active" aria-current="page">Corretora SMS</li>
                                    </li>
                                </ol>
                            </nav>
                        </div>
            ...
            <div class="container-fluid">
                ...
                <!-- FIM CARD ADICIONAR CORRETORA SMS -->        
            </div>
            <!-- ============================================================== -->
            <!-- End Container fluid  -->
            <!-- ============================================================== -->
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 m-b-15 responsive">
                    ...
                    <i class="fas fa-plus"></i> <span class="icon-name">Nova Corretora SMS</span>
                    </a>        
                                ...
                                <div class="header">
                                    <div class="row clearfix">
                                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                            <h2>Adicionar Nova Corretora SMS</h2>
                                        </div>
                                    </div>
                                </div>
                                <div class="body">
                                        ...
                                        <tr id="tr-{{$broker['id']}}">
                                            ...
                                            <td>
                                                <div class="demo-checkbox">
                                                    <input type="checkbox" id="broker_active{{$broker['id']}}" {{ $broker['active'] ? 'checked' : ''}} onchange="active_broker(this, {{$broker['id']}})" name="broker_active{{$broker['id']}}" class="chk-col-indigo" style="margin-right: 5px;"/>
                                                    <label id="label-{{$broker['id']}}" for="broker_active{{$broker['id']}}">{{ $broker['active'] ? 'ativo' : 'inativo'}}</label>
                                                </div>
                                            </td>
                                            <td>
                                                <a class='btn btn-primary' onclick="changepage('{{route('brokersms.edit', ['brokersm'=>$broker['id']])}}')">Atualizar</a>
                                                <button type='button' class='btn btn-secondary' style='margin-left: 5px;' id="btn-delele-broker-{{$broker['id']}}" onclick="deleteBrokerSMS({{$broker['id']}})">Deletar</button>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
    ...
    <script>
        ...
        function deleteBrokerSMS(id) {
            $(`#btn-delele-broker-${id}`).text('Deletando...');
            $(`#btn-delele-broker-${id}`).prop('disabled', true);
            var baseUrlDelete = "{{route('brokersms.destroy', ['brokersm'=>'idUser'])}}";
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.error == 500) {
                        ...
                        changepage("{{route('brokersms.index')}}");
                        return false
                    }
                    if (result['error']) {
                        swal(
                            'Falha',
                            'Erro ao excluir corretora sms',
                            'warning'
                        );    
                    }
                    swal(
                        'Sucesso',
                        'Corretora sms excluído com sucesso',
                        'success'
                    );
                    changepage("{{route('brokersms.index')}}");
                },
                error: function (result) {
                    ...
                    $(`#btn-delele-broker-${id}`).text('Deletar');
                    $(`#btn-delele-broker-${id}`).prop('disabled', false);
                }
            });
        }
        function active_broker(e, id) {
            ...
            $('#idNovoRegistro').submit(function(e){
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        ...
                        swal(
                            'Sucesso',
                            'Corretora SMS cadastrado com sucesso',
                            'success'
                        );
                        changepage("{{route('brokersms.index')}}")
                    },
